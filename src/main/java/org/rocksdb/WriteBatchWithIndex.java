...
/**
 * Similar to {@link WriteBatch} but with a binary searchable
 * index built for all the keys inserted.
 *
 * Calling put, merge, remove or putLogData calls the same function
 * as with {@link WriteBatch} whilst also building an index.
 *
 * A user can call {@link WriteBatchWithIndex#newIterator()} to
 * create an iterator over the write batch or
 * {@link WriteBatchWithIndex#newIteratorWithBase(RocksIterator)}
...
public class WriteBatchWithIndex extends AbstractWriteBatch {
   ...
   * {@link RocksIteratorInterface#seek(byte[])} to
   ...
   * creating a new Iterator that will use {@link WBWIRocksIterator}
